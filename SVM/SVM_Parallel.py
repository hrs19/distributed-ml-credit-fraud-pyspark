...
# print('scaled data:',training_data_rdd.take(2))

# Train the SVM model
fit_time = time()
svm_model = SVMWithSGD.train(training_data_rdd, iterations=10000, step=)
fit_time = time()-fit_time

predictions_and_labels_rdd = test_data_rdd.map(lambda row: (float(svm_model.predict(row.features)), row.label))

print('Predictions:',predictions_and_labels_rdd.take(5))
...
print(f'Time taken: \nReadDataRDD: {read_data_time}\tScale Train set: {scale_train_time}\tScale Test set: {scale_test_time}\tSVMWithSGD model train: {fit_time}')
sc.stop()
